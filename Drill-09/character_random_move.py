...
def update_character():
    ...
    # 다음 코드가 하는 일은?
    # 첫 점에서 다음 점까지 도착하게 되면, 첫 점을 버리고 새로운 점을 받아온다.
    #예) 0, 1, 2, 3에서 1에 도착 시, 새로 점을 받아오는데 1, 2, 3, 4 를 받아온다.
    t += 0.001
    if t >= 1.0:
    ...
    # 다음 코드의 목적은?
    #이전 좌표에서 좌측으로 가는지, 우측으로 가는지 구분하여 캐릭터가 바라보는 방향을 바꿔준다.
    running_right = cx > prev_cx
    prev_cx = cx


# 아래 코드는 어떤 식으로 4개의 포인트를 가져오는가?
# 현재 캐릭터가 있는 점(start)에서부터 이후 4개의 점을 받아온다.
...
# 4개씩 점을 불러오기 때문에 8번 점에서 없는 11번째 점을 가져오려고 해서 문제가 생기기에 그 문제를 없애주고자 0부터 2번까지를 뒤에 다시 붙여준 것이다.
extended_target_points = target_points + target_points[:3] # 앞의 3개를 뒤에 다시 붙여줌
cur_index = -1
